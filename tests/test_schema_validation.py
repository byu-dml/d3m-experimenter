...
class TestSchemaValidation(unittest.TestCase):
    ...
    def test_valid_pipeline_schema(self):
        ...
        try:
            ...
            self.fail("Valid pipeline schema failed to validate because of {}".format(error))

    def test_invalid_pipeline_schema(self):
        with open(os.path.join(self.pipeline_path, "invalid_pipeline.json"), 'r') as file:
            pipeline_to_check = pipeline_module.Pipeline.from_json(string_or_file=file)

        try:
            pipeline_to_check.check()
            self.fail("Invalid pipeline schema validated when it should have FAILED")
        except Exception:
            pass
