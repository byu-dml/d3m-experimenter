...
class PipelineGenerationTestCase(unittest.TestCase):
    ...
    def test_arch_desc(self) -> None:
        ...
        self.assertEqual(
            arch_desc_json["generation_parameters"]["other_test_attr"], "abc"
        )
    ...
    def test_concat_cache(self) -> None:
        ...
        pipeline.add_primitive_step("d3m.primitives.feature_extraction.pca.SKlearn")
        pipeline.set_step_i_of("pca")

        first_concat_ref = pipeline.concatenate_inputs(
            pipeline.data_ref_of("scaler"), pipeline.data_ref_of("pca")
        )
    ...
    def test_classifier_post_processing(self) -> None:
        ...
        step_paths = [
            step.primitive.metadata.query()["python_path"] for step in pipeline.steps
        ]
        ...
        self.assertIn("d3m.primitives.data_cleaning.label_encoder.DSBOX", step_paths)
