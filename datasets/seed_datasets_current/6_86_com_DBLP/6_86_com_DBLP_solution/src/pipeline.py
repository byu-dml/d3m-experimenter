...
dspath = os.path.join(here, '..', '..', '6_86_com_DBLP_dataset')
prpath = os.path.join(here, '..', '..', '6_86_com_DBLP_problem')
...
def read_graph():
	graphPath = os.path.join(d3mds.dataset.dsHome, 'graphs', 'G1.gml')
...
def make_intermediate_results():
	...
	try:
		...
		if len(clusters) > len(nodes):
			nodes = nodes + [0]*(len(clusters)-len(nodes))
			idxs = list(range(1, len(nodes)+1))
		elif len(nodes) > len(clusters):
			clusters = clusters + [0]*(len(nodes)-len(clusters))
