...
def evaluate_pipeline_on_problem(
    ...
    TODO doc
    """
    dataset_digest = sha256(get_dataset(dataset_path)).hexdigest()
    problem_digest = sha256(get_problem(problem_path)).hexdigest()
    pipeline_digest = sha256(get_pipeline(pipeline_path)).hexdigest()
    
    ouput_run = '-'.join([dataset_digest, problem_digest, pipeline_digest])

    evaluate_pipeline_via_d3m_cli(pipeline=pipelin_path,
        problem=problem_path,
        input=dataset_path,
        output_run=output_run)

# def run_pipeline(pipeline: Pipeline, )
# one-level up function to run pipeline and post to metadatabase
